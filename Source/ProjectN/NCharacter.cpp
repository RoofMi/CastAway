...
void ANCharacter::Interaction()
{
	if (!IsScriptOpened) {
		...
		// 파도 소리 기능
		else if (LockWaveSound != nullptr)
		{
			NLOG(Warning, TEXT("WaveSound Interaction"));
			NAnim->SetWhistle();
			...
			LockWaveSound->PlaySound();
			LockWaveSound->Destroy();
			Lock = false;
			LockWaveSound = nullptr;
			CanSoundwave = false;
			return;
		...
		else
		{
			if (!IsCoconut && CanCoconut) {
				...
				if ((GetActorLocation() - tree).Size() <= 175) {
					GetWorld()->SpawnActor<ANCoconut>(FVector(860, -110, 450), FRotator::ZeroRotator);
					IsCoconut = true;
					//CanCoconut = false;
				}
			}
	...
	else
	{
		...
		// 감정 수치 변동
		SetDamage(CurrentScript->Joy2, CurrentScript->Sad2, CurrentScript->Anger2, CurrentScript->Fear2);
		// 읽은 스크립트 개수 변동
		CharacterStat->SetSurvivalDays(CharacterStat->GetSurvivalDays() + 1);
		Controller->GetHUDWidget()->UpdateScriptCount(CharacterStat->GetSurvivalDays());
...
void ANCharacter::InteractionLock()
{
	...
	// 파도 소리 상호작용
	for (ANWaveSound* Wave : TActorRange<ANWaveSound>(GetWorld()))
	{
		if ((GetActorLocation() - Wave->GetActorLocation()).Size() <= 150 && !Lock)
		{
			Lock = true;
			LockWaveSound = Wave;
			return;
	...
	if ((LockWaveSound != nullptr) && (GetActorLocation() - LockWaveSound->GetActorLocation()).Size() > 300)
	{
		Lock = false;
		LockWaveSound = nullptr;
	}
	...
	else
	{
		...
		if (branch == 1) {
			if (MainXCount == 0)
			{
				...
				NLOG(Warning, TEXT("TriggerBox0 deleted!!!"));
				// 시점 확대
			...
			{
				// 파도소리 기능 활성화
				ANWaveSound* SpawnedActor = (ANWaveSound*)GetWorld()->SpawnActor<ANWaveSound>(FVector(650.0f, 300.0f, 100.0f), FRotator::ZeroRotator);
				CanSoundwave = true;
